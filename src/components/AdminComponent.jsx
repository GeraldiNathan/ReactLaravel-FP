...
function AdminPage() {
  ...
  return (
    <div class="flex flex-col justify-center items-center mt-14">
      ...
      {updateFlashMessage && (
        <div
          className={`fixed left-1/2 transform -translate-x-1/2 z-50 p-4 mb-4 rounded-md ${updateFlashMessage.color === "green" ? "bg-green-100 border border-green-400 text-green-700" : "bg-red-100 border border-red-400 text-red-700"}`}
          role="alert"
        >
          <div className="flex items-center justify-between">
            {" "}
            <div className="flex items-center">
              {" "}
              <strong className="font-bold">{updateFlashMessage.color === "green" ? "Success! " : "Error!"}</strong>
              <span className="block sm:inline ml-2">{updateFlashMessage.message}</span>
            </div>
            <span className="ml-4" onClick={() => setUpdateFlashMessage(null)}>
              {" "}
              <svg className={`fill-current h-6 w-6 ${updateFlashMessage.color === "green" ? "text-green-500" : "text-red-500"}`} role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                <title>Close</title>
                <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z" />
              </svg>
            </span>
            ...
            <table class="max-w-full ">
              ...
              <tbody>
                {currentData.map((data) => (
                  <tr key={data.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-black dark:text-black">{data.id}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-black">{data.title}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-black">{data.description}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                      <button
                        ...
                        onClick={() => document.getElementById(`update_modal_${data.id}`).showModal()}
                      >
                        Update
                      </button>
                      <dialog id={`update_modal_${data.id}`} className="modal rounded-2xl">
                        <div className="p-6">
                          <h2 className="text-2xl font-bold mb-4 text-left">Update Recipe</h2>
                          <div className="mb-6">
                            <label htmlFor="recipe_title" className="block text-sm text-left font-medium text-amber-500 ">
                              Title
                            </label>
                            <input
                              type="text"
                          ...
                          <div className="mb-6">
                            <label htmlFor="recipe_description" className="block text-sm font-medium text-amber-500  text-left">
                              Description
                            </label>
                            <textarea
                          ...
                          <div className="mb-6">
                            <label htmlFor="choose_image" className="text-left block text-sm font-medium text-amber-500 ">
                              Choose Image
                            </label>
                            <input
                              type="file"
                              name="file_input"
                              id="choose_image"
                              className={`w-full p-2 border-b-2 ${formData.file_path ? "border-amber-200 text-green-700" : "border-amber-200 text-red-400"} focus:outline-none focus:border-amber-500`}
                              required
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                      <button
                        ...
                        onClick={() => document.getElementById(`delete_modal_${data.id}`).showModal()}
                      >
                        Delete
                      </button>
                      <dialog id={`delete_modal_${data.id}`} className="modal rounded-2xl">
